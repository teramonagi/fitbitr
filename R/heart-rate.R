...
#' @export
get_heart_rate_intraday_time_series <- function(token, date="", detail_level="1min", base_date="", end_date="", start_time="", end_time="")
{
  url <- if(date!=""){
    if(start_time != "" && end_time != ""){
      paste0(url_heart, sprintf("date/%s/1d/%s/time/%s/%s.json", format_date(date), detail_level, start_time, end_time))
    } else{
      paste0(url_heart, sprintf("date/%s/1d/%s.json",            format_date(date), detail_level))
    }
  } else{
    if(start_time != "" && end_time != ""){
      paste0(url_heart, sprintf("date/%s/%s/%s/%s/%s.json", format_date(base_date), format_date(end_date), detail_level, start_time, end_time))
    } else{
      paste0(url_heart, sprintf("date/%s/%s/%s.json",       format_date(base_date), format_date(end_date), detail_level))
    }
  }
