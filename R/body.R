...
#' @export
get_body_fat_logs <- function(token, date="", base_date="", period="", end_date="")
{
  url <- if(date != "" && period != ""){
...
log_body_fat <- function(token, fat, date, time=NULL)
{
  ...
  body <- if (!is.null(time)){
    list(fat=fat, date=format_date(date), time=time)
  } else {
...
#' @export
update_body_fat_goal <- function(token, goal)
{
  url <- paste0(url_body, "log/fat/goal.json")
  response <- post(url, token, body=list(fat=goal))
  convert_content_to_r_object(response)
}
...
#' Get Weight Logs
#'
#' The Get Weight Logs endpoint returns time series data in the specified range for a given resource in the format requested using units in the unit system that corresponds to the Accept-Language header provided.
#'
