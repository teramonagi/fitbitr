...
#'
#' @param token An OAuth 2.0 token
#' @param resource-path	The resource path; see the Resource Path Options below for a list of options.
...
{
  url <- paste0(url_sleep, sprintf("%s/date/%s/%s.json", resource_path, date, period))
  response <- get(url, token)
  data <- convert_content_to_r_object(response)
  data[[1]]
}
